...
const AddModuleModal = (props) => {
    ...
    return (
        <>
            <Button 
                color="blue-gray" variant="text" 
                className="flex items-center h-10 justify-center mx-2 border border-gray-400/70"
                onClick={() => setVisible(true)}
            >
                <PlusIcon className="h-5 w-5 text-center" />
            </Button>
            <Dialog open={visible} handler={setVisible} size="md" className="flex flex-row bg-transparent shadow-none items-center justify-center">
                <Card className="relative flex w-2/4 rounded-lg overflow-y-auto scrollbar-thin scrollbar-thumb-rounded-xl scrollbar-thumb-blue-gray-100">
                    <CardBody className="mb-2">
                        ...
                        <Typography className="mb-3">
                            We'll need a concise name for it first! Please enter a module
                            name below. 
                        </Typography>
                        <Input 
                            variant="outlined" label="Module Name" 
                            color="blue-gray"
