import { createSlice, nanoid } from "@reduxjs/toolkit";

// TODOListSlice sets up the initial redux state, defines reducers for adding,
// editing, and deleting TODO items

const INITIAL_STATE = {
  categories: [
    "Biology",
    "Chemistry",
    "Computer Science",
    "Language Arts",
    "Math",
    "Musical Art",
    "Physics",
    "Sports",
    "Visual Arts",
    "Work",
  ],
  TODOList: [
    {
      id: "1",
      title: "Complete math hwk",
      dueDate: "June 5, 2023",
      description: "This is a sample todo.",
      category: "Math",
    },
    {
      id: "2",
      title: "Complete physics hwk",
      dueDate: "June 2, 2023",
      description: "This is a sample todo.",
      category: "Physics",
    },
    {
      id: "3",
      title: "Complete English literature hwk",
      dueDate: "June 24, 2023",
      description: "This is a sample todo.",
      category: "Language Arts",
    },
    {
      id: "4",
      title: "Complete math midterm",
      dueDate: "July 14, 2023",
      description: "This is a sample todo.",
      category: "Math",
    },
    {
      id: "5",
      title: "Complete math Final",
      dueDate: "Aug 5, 2023",
      description: "This is a sample todo.",
      category: "Math",
    },
    {
      id: "6",
      title: "Complete Physics Final",
      dueDate: "July 10, 2023",
      description: "This is a sample todo.",
      category: "Physics",
    },
    {
      id: "7",
      title: "Complete Physics Final",
      dueDate: "Aug 12, 2023",
      description: "This is a sample todo.",
      category: "Physics",
    },
    // Add more sample todos here
  ],
};

const TODOListSlice = createSlice({
  name: "todoList",
  initialState: INITIAL_STATE,
  reducers: {
    // The addTODO reducer is responsible for adding a new TODO item to the
    // TODOList array in the state.
    addTODO: {
      reducer: (state, action) => {
        state.TODOList.push(action.payload);
      },
      // The prepare function is a pre-reducer that prepares the payload
      // for the addTODO reducer by generating a unique ID using
      // nanoid() and merging it with the provided todo object.
      prepare: (todo) => {
        return {
          payload: {
            id: nanoid(),
            ...todo,
          },
        };
      },
    },
    editTODO: (state, action) => {
      const { id, title, dueDate, description, category } = action.payload;
      const todoIndex = state.TODOList.findIndex((todo) => todo.id === id);
      if (todoIndex !== -1) {
        state.TODOList[todoIndex] = {
          id,
          title,
          dueDate,
          description,
          category,
        };
      }
    },

    // deleteTODO finds the index of the item based on its ID
    // and deletes it using the splice method.
    deleteTODO: (state, action) => {
      const todoIndex = state.TODOList.findIndex(
        (todo) => todo.id === action.payload
      );
      if (todoIndex !== -1) {
        state.TODOList.splice(todoIndex, 1);
      }
    },
    deleteAllTODOs: (state) => {
      state.TODOList = [];
    },
    addCategory: (state, action) => {
      const categoryIndex = state.categories.findIndex(
        (item) => item === action.payload
      );

      // Only add payload to state.categories if category does not already
      // exist in the array
      if (categoryIndex === -1) {
        state.categories.push(action.payload);
        state.categories.sort();
      }
    },
    // removes the current todoItem category in category list
    deleteCategory: (state, action) => {
      // remove current category from category list
      const categoryPos = state.categories.findIndex(
        (category) => category === action.payload
      );
      if (categoryPos !== -1) {
        state.categories.splice(categoryPos, 1);
      }

      // remove all todoItems with the deleted category
      state.TODOList = state.TODOList.filter((todo) => {
        return todo.category !== action.payload;
      });
    },
  },
});

// Exports the action creators generated by the createSlice function as named exports.
// These action creators can be used to dispatch actions to modify the TODO list state.
export const {
  addTODO,
  editTODO,
  deleteTODO,
  deleteAllTODOs,
  addCategory,
  deleteCategory,
} = TODOListSlice.actions;

// The TODOListSlice.reducer is exported as the default export, representing the reducer
// function for this slice.
export default TODOListSlice.reducer;
